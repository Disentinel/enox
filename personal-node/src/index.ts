...
async function main() {
  ...
  // Agent-native share links: public landing page + per-share MCP endpoint.
  // Auth here is each share's own token (src/shares/auth.ts), never requireAuth —
  // must stay outside the /mcp,/sse,/sse/messages requireAuth block above, and
  // must be registered before the SPA catch-all below.
  mountShareRoutes(app);

  // OAuth discovery probes must 404, not fall through to the SPA catch-all:
  // a 200 here makes claude.ai treat the server as OAuth-protected and break
  // token-auth connectors (observed 2026-07-22).
  app.get(['/.well-known/oauth-protected-resource',
           '/.well-known/oauth-protected-resource/{*path}',
           '/.well-known/oauth-authorization-server',
           '/.well-known/oauth-authorization-server/{*path}',
           '/.well-known/openid-configuration'], (_req, res) => {
  ...
  const server = app.listen(config.port, () => {
    ...
    console.log(`[enox] Auth: ${config.authToken ? 'enabled' : 'disabled (dev mode)'}`);
  });
