...
const ENTITY_COLS = 'e.id AS id, e.type AS type, e.domain AS domain, e.name AS name, e.description AS description, e.aliases AS aliases, e.source_ref AS source_ref, e.created_at AS created_at, e.updated_at AS updated_at';
...
export async function listNodes(req: Request, res: Response) {
  ...
  // Stable secondary sort on the PRIMARY KEY (e.id) so rows with identical
  // updated_at have a total, deterministic order across paginated calls —
  // otherwise a concurrent write bumping updated_at can push a domain's rows
  // off page 1 and produce a false "empty" read.
  cypher += ' ORDER BY e.updated_at DESC, e.id ASC';
...
export async function createNode(req: Request, res: Response) {
  ...
  const { type, domain, name, description, aliases, source_ref } = parsed.data;
  ...
  await execute(
    ...
    { id, type, domain, name, description: description ?? '', aliases, source_ref: source_ref ?? '', now },
  );

  res.status(201).json({ id, type, domain, name, description: description ?? '', aliases, source_ref: source_ref ?? '', created_at: now, updated_at: now });
}
