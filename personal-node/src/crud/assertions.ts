...
import { computeFactId, withFactIdLock } from '../util.js';
import { searchSimilar } from '../embeddings.js';

// asserted_by is required on creation (CreateAssertionSchema — no default,
// no anonymous facts via the REST API). proof_depth stays optional/default 0;
// retire-vs-revive for it is an open decision, not made here (2026-07-24).
...
export async function listAssertions(req: Request, res: Response) {
  const { source, target, relation, domain, limit, offset } = req.query;
  ...
  // Server-side domain filter on the source node's domain, applied in the
  // WHERE (before SKIP/LIMIT) so we page the FILTERED set — not page-1-of-
  // everything then filter client-side, which yields false "empty" reads.
  if (domain) {
  ...
  // Stable secondary sort on fact_id (effectively unique per relationship —
  // deterministic computeFactId + withFactIdLock guard against dup rows) so
  // rows with identical updated_at have a total, deterministic order across
  // paginated calls and don't get pushed off page 1 by concurrent writes.
  cypher += ' ORDER BY r.updated_at DESC, r.fact_id ASC';
...
export async function createAssertion(req: Request, res: Response) {
  ...
  const { source, target, relation, confidence, context, asserted_by, proof_depth } = parsed.data;
  ...
  // withFactIdLock serializes concurrent check-then-create calls for the same
  // fact_id within this process — Kuzu's Assertion rel table has no unique
  // constraint on fact_id (only node tables support PRIMARY KEY), so this
  // existing-row check is the only guard against duplicate rows, and without
  // the lock two concurrent POSTs for the same triple could both see "not
  // found" before either CREATE runs.
  const outcome = await withFactIdLock(fact_id, async () => {
    ...
    return { conflict: false as const, now };
  });
...
export async function traverseGraph(req: Request, res: Response) {
  ...
  const startNode = await queryOne<{ id: string; name: string; type: string; domain: string; source_ref: string }>(
    'MATCH (e:Entity) WHERE e.id = $id RETURN e.id AS id, e.name AS name, e.type AS type, e.domain AS domain, e.source_ref AS source_ref',
    { id },
  );
  if (!startNode) { res.status(404).json({ error: 'Node not found' }); return; }

  type EdgeRow = { src: string; src_name: string; tgt: string; tgt_name: string; tgt_type: string; relation: string; asserted_by: string; confidence: number; context: string };
      ...
      if (direction === 'out' || direction === 'both') {
        const rows = await queryAll<EdgeRow>(
          `MATCH (a:Entity)-[r:Assertion]->(b:Entity) WHERE a.id = $id${relCond}
           RETURN a.id AS src, a.name AS src_name, b.id AS tgt, b.name AS tgt_name, b.type AS tgt_type, r.relation AS relation, r.asserted_by AS asserted_by, r.confidence AS confidence, r.context AS context`,
          params,
        );
  ...
  for (const { id, score, match_type } of similar) {
    ...
    const edge = await queryOne(
      `MATCH (a:Entity)-[r:Assertion]->(b:Entity) WHERE r.fact_id = $fid
       RETURN a.name AS source_name, a.id AS source_id, a.domain AS source_domain, b.name AS target_name, b.id AS target_id, r.relation AS relation, r.asserted_by AS asserted_by, r.confidence AS confidence, r.context AS context`,
      { fid: factId },
    );
    if (!edge) continue;
...
export async function getNeighbors(req: Request, res: Response) {
  ...
  const node = await queryOne(
    'MATCH (e:Entity) WHERE e.id = $id RETURN e.id AS id, e.type AS type, e.domain AS domain, e.name AS name, e.description AS description, e.aliases AS aliases, e.source_ref AS source_ref, e.created_at AS created_at, e.updated_at AS updated_at',
    { id },
  );
  if (!node) {
  ...
  const outgoing = await queryAll(
    `MATCH (a:Entity)-[r:Assertion]->(b:Entity) WHERE a.id = $id
     RETURN r.fact_id AS fact_id, a.id AS source, b.id AS target, b.name AS target_name, b.type AS target_type,
            r.relation AS relation, r.asserted_by AS asserted_by, r.confidence AS confidence, r.context AS context, r.created_at AS created_at, r.updated_at AS updated_at`,
    { id },
  );
