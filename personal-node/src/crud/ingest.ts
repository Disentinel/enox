...
const PROMPT_TEMPLATE = `You are an ENOX knowledge graph extractor. You receive a short free-text note from the user about their knowledge graph. Extract entities and relations.

## Context
The user is looking at their personal knowledge graph and adding a note. The note may reference existing entities, add opinions, create new concepts, or draw connections.

## URI references
If the note contains full entity URIs like enox://example.org/graph/main/cs/knowledge_graph — use that exact ID in edges (strip the prefix, use just cs/knowledge_graph). Do NOT create a new entity for it.
...
export async function ingest(req: Request, res: Response) {
  const { text, selectedNode, assertedBy } = req.body as {
    text?: string;
    selectedNode?: { id: string; name: string; description?: string };
    /** Explicit asserter identity for this note's edges. Falls back to config.agentIdentity — this pipeline runs an LLM extractor, it's agent-asserted, not user-asserted, unless told otherwise. */
    assertedBy?: string;
  };
  ...
  const effectiveAssertedBy = assertedBy?.trim() || loadConfig().agentIdentity;
        ...
        if (!existing) {
          ...
          await execute(
            'CREATE (:Entity {id: $id, type: $type, domain: $domain, name: $name, description: $description, aliases: $aliases, source_ref: $source_ref, created_at: $now, updated_at: $now})',
            {
              ...
              // The extractor prompt doesn't currently ask the LLM for a
              // source URL/DOI, so this is almost always empty for
              // freetext-note extraction — that's honest: there IS no
              // external source for "the user jotted a note", unlike an
              // ingested paper/URL. Populate via update_node when a real
              // source_ref is known.
      ...
      try {
        ...
        await execute(
           ...
           CREATE (a)-[:Assertion {fact_id: $fact_id, relation: $relation, asserted_by: $asserted_by, confidence: $confidence, proof_depth: $proof_depth, context: $context, created_at: $now, updated_at: $now}]->(b)`,
          {
            ...
            proof_depth: (obj.proof_depth as number) ?? 0,
