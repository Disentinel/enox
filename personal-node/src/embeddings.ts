...
import { queryAll, execute } from './db/kuzu.js';
...
async function getEmbedder() {
  ...
  if (!loadingPromise) {
    loadingPromise = (async () => {
      ...
      console.error('[embed] Model load failed:', err);
...
// Embed arbitrary text — exported so other stores (e.g. share snapshots) can
// run semantic search against their own vector maps using the same model.
...
// Extract a subset of the in-memory embedding store for a given set of keys
// (node IDs and/or "assertion:<fact_id>" keys). Used to copy embeddings into
// a share snapshot cheaply, without touching the main store.
export function getEmbeddingsSubset(keys: Set<string>): Record<string, number[]> {
...
export async function searchSimilar(
  query: string,
  topK = 10,
): Promise<Array<{ id: string; score: number; match_type: 'node' | 'assertion' }>> {
  if (store.size === 0) return [];

  const qVec = await embed(query);
  const results: Array<{ id: string; score: number; match_type: 'node' | 'assertion' }> = [];

  for (const [id, vec] of store) {
    const score = cosineSim(qVec, vec);
    results.push({ id, score, match_type: id.startsWith('assertion:') ? 'assertion' : 'node' });
  }
...
// Background worker: find unembedded nodes and assertions, process them
async function processUnembedded(): Promise<number> {
  ...
  for (const node of allNodes) {
    ...
    try {
      ...
      if (processed % 20 === 0) {
        saveEmbeddings();
        console.log(`[embed] Processed ${processed} items...`);
      }
    } catch (err) {
  ...
  // Pass 2: Embed assertions with non-empty context
  const allAssertions = await queryAll<{
    fact_id: string;
    source_name: string;
    target_name: string;
    relation: string;
    ...
    `MATCH (a:Entity)-[r:Assertion]->(b:Entity)
     ...
     RETURN r.fact_id AS fact_id, a.name AS source_name, b.name AS target_name, r.relation AS relation, r.context AS context`,
  );

  for (const edge of allAssertions) {
    const key = `assertion:${edge.fact_id}`;
    if (store.has(key)) continue;

    const text = `${edge.source_name} ${edge.relation} ${edge.target_name}: ${edge.context}`;
    try {
      const vec = await embed(text);
      store.set(key, vec);
      processed++;
      ...
      console.error(`[embed] Failed to embed assertion ${edge.fact_id}:`, err);
    }
  }
