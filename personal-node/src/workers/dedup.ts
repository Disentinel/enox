...
const VALID_RELATIONS = new Set(RELATION_TYPES as readonly string[]);
...
/**
 * The LLM judge is OPTIONAL. It shells out to the `claude` CLI (see Dockerfile),
 * which needs an authenticated session (`CLAUDE_CODE_OAUTH_TOKEN`). When no token
 * is configured — the default for a fresh self-host — we skip this step entirely:
 * the node still finds and merges duplicates deterministically, it just doesn't
 * emit LLM-suggested links. This must never crash the worker or the process.
 */
...
async function judgeLinkCandidates(pairs: CandidatePair[]): Promise<LinkJudgement[]> {
  ...
  if (!llmJudgeEnabled()) {
    console.log('[dedup] LLM judge disabled (no CLAUDE_CODE_OAUTH_TOKEN / ANTHROPIC_API_KEY) — skipping link suggestions');
    return [];
  }
...
Return [] if none should be linked.`;
  ...
  try {
    ...
    for (const j of judgements) {
      ...
      if (!VALID_RELATIONS.has(j.relation)) continue;
...
export async function runDedupCycle(): Promise<{ candidates: number; merged: number; linked: number; results: MergeResult[] }> {
  ...
  try {
    ...
    if (toLinkJudge.length > 0 && llmJudgeEnabled()) {
