...
function detectBasePath(): string {
  ...
  const spaRoutes = ['login', 'dashboard', 'explorer', 'graph', 'timeline', 'queue', 'perspectives', 'metrics', 'pipelines', 'wiki'];
...
// Artifacts — wiki pages, docs, and other files stored via POST /api/artifacts
...
export interface ArtifactDetail extends ArtifactRecord {
  links: ArtifactEdge[];
  incoming: ArtifactEdge[];
}

export const listArtifacts = (params?: { domain?: string; content_type?: string; limit?: number; offset?: number }) => {
  const qs = new URLSearchParams();
  if (params?.domain) qs.set('domain', params.domain);
  if (params?.content_type) qs.set('content_type', params.content_type);
  if (params?.limit) qs.set('limit', String(params.limit));
  if (params?.offset) qs.set('offset', String(params.offset));
  return request<ArtifactRecord[]>('GET', `/api/artifacts?${qs}`);
};

export const getArtifact = (id: string) =>
  request<ArtifactDetail>('GET', `/api/artifacts/${encodeURIComponent(id)}`);

// Raw body isn't JSON (it's the artifact's own content-type), so this bypasses
// the shared request() helper and reads text directly.
export const getArtifactRaw = async (id: string): Promise<string> => {
  const headers: Record<string, string> = {};
  const token = getToken();
  if (token) headers['Authorization'] = `Bearer ${token}`;

  const res = await fetch(`${BASE}/api/artifacts/${encodeURIComponent(id)}/raw`, { headers });
