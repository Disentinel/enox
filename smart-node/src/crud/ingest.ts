...
const PROMPT_TEMPLATE = `You are an ENOX knowledge graph extractor. You receive a short free-text note from Vadim about his knowledge graph. Extract entities and relations.

## Context
Vadim is looking at his personal knowledge graph and adding a note. The note may reference existing entities, add opinions, create new concepts, or draw connections.

## URI references
If the note contains full entity URIs like enox://enox.dev/personal/vadim_r/cs/knowledge_graph — use that exact ID in edges (strip the prefix, use just cs/knowledge_graph). Do NOT create a new entity for it.
